...
class PickPlace(SingleArmEnv):
    ...
    def __init__(
        ...
        novelties_on=False,
        #activate_pos=(-0.25, 0.2, 0.32),
        #lightswitch_pos=(-0.25, -0.02, 0.32),
        activate_pos=(0.05, 0.2, 0.32),
        lightswitch_pos=(-0.25, 0.2, 0.32),
        cylinder_pos=(0.15, 0.27, 0.1),
        plate_pos=(-0.4, 0.525, 0.1),
        door_pos=(-10.08, -0.26, 0.1, -3*np.pi/2),
        ...
        # Novelties settings
        self.novelties_on = novelties_on or novelty is not None
        if self.novelties_on:
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            if self.novelties_on:
                ...
                door_pos, door_quat, _ = door_placement
...
class PickPlaceCanNovelties(PickPlace):
    """
    Easier version of task - place one can into its bin.
    """
    ...
    def __init__(self, **kwargs):
        assert "single_object_mode" not in kwargs and "object_type" not in kwargs, "invalid set of arguments"
        super().__init__(single_object_mode=2, object_type="can", novelties_on=True, **kwargs)